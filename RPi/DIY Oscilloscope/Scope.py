...
pygame.init()
os.environ['SDL_VIDEO_WINDOW_POS'] = 'centre'
pygame.display.set_caption("DIY Pi-Arduino OScilloscope")
...
screenWidth =720; screenHeight = 360;
...
logo = pygame.image.load("Add Address of image here").convert_alpha()

sampleInput = serial.Serial("/dev/ttyACM0", 115200, timeout = 5)

displayWidth = 512; displayHeight = 256
LedRect = [ pygame.Rect((0,0),(0,0))] * 17
inBuf = [0]*512 # 512 long buffer
chOff = displayHeight//2
run   = [True, False, False, True, False]
expandT = 1; expandV =1 # voltage and time expansion

sampleTime = 17
sample_cm =32 * sampleTime
volts_sample = 5/1024
measureTime = False; measureVolts = False; savedTime= 0; savedVoltage = 0
cursotT = 0; cursorV = 0; vMag =1; svLed = False; strLed =False
triggerC = 512; savedVoltsC = -1; savedTimeC = -1

def main():
    pygame.draw.rect(screen, backCol, (0,0,screenWidth, screenHeight+2), 0 )
    defineControls()
    drawControls()
    time.sleep(0.1)
    sampleInput.flushInput()
...
def drawGrid():
    pygame.draw.rect(display, (240,240,240), (0,0,displayWidth, displayHeight), 0)

    for h in range(32,256,32) :
        pygame.draw.line( display, (120,120,120), (0,h), (512,h), 1)

    for v in range(32, 512, 32) :
...
def updateControls(blank):
    ...
    if expandT*sample_cm >= 1000:
        drawWords("Time"+str((expandT*sample_cm)//1000)+"ms per division ", 10, 280, black, backCol)
    else:
        drawWords("Time"+str(expandT*sample_cm)+"us per division ",10,280,black,backCol)
...
def defineControls():
    ...
    LedRect[10] = pygame.Rect((486,336),(15,15)) # volts        LedRect[11] = pygame.Rect((540,336),(15,15)) # save time
    LedRect[12] = pygame.Rect((586,336),(15,15)) # save volts
    LedRect[13] = pygame.Rect((545,100),(15,15)) # run        LedRect[14] = pygame.Rect((580,100),(15,15)) # single
